...
-- OneCare demo data
--
-- Fills the surfaces that a walkthrough of the demo accounts found empty:
-- healthcare instructions, the catch-up tabs, secure messages, the clinician's
-- triage inbox and task list, patient activity, and lab-type results for the
-- analytics charts.
--
-- Run in the Supabase SQL editor. Safe to run repeatedly — it clears the rows it
-- previously created before inserting, so it never doubles up.
--
-- It refuses to run against anything but demo-*@onecare.you accounts. Read the
-- guard at the top before changing that.
--
-- NOT included: Health Vault documents. Those need real files in storage, which
-- SQL cannot upload. See demo_vault_documents.sql and the upload step.
-- =============================================================================

DO $$
DECLARE
  _james    uuid;   -- demo-patient-1  James Thompson
  _emily    uuid;   -- demo-clinician-1 Dr Emily Williams
  _third    uuid;   -- demo-clinician-3
  _share    uuid;
  _now      timestamptz := now();
  _bad      integer;
BEGIN
  -- ---------------------------------------------------------------------------
  -- Guard: demo accounts only.
  -- ---------------------------------------------------------------------------
  SELECT count(*) INTO _bad
    FROM auth.users
   WHERE email IN ('demo-patient-1@onecare.you',
  ...
  -- ---------------------------------------------------------------------------
  -- Clear anything this script created before, so re-running is safe.
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 1. Healthcare instructions, spread across the catch-up tabs
  --    (pending / acknowledged / completed all need to show something)
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 2. A secure message thread with something unread at the end
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 3. Alerts for the triage inbox — two waiting, two already handled
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 4. The clinician's own task list
  --    practice_id is null: these are personal tasks, visible only to Emily.
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 6. Lab-type results, so the analytics charts have something to draw
  --    A believable picture: HbA1c improving, lipids borderline, kidneys stable.
  -- ---------------------------------------------------------------------------
  ...
  RAISE NOTICE 'Demo data seeded for James Thompson and Dr Emily Williams.';
  RAISE NOTICE 'James user_id = %  (needed for the Health Vault upload step)', _james;
END $$;
