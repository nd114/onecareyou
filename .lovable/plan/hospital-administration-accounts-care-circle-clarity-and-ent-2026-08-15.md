...
- **Account separation**: an administrative account is its own login, issued to one named person. It gets no patient dashboard and no clinical record surface; the header, nav, and route guards reflect that. Every administrative action is written to the audit log with the acting account, so credentials are never shared. Pagination included in audit log. 
...
- Create a demo hospital with hospital owner, sub admins, and 20 dummy patients. This would be quite a lot of demos, and issue them all the same password Demo123! as we have. This is so when showing and training LMC they can see how the flow works, and for us to also conduct testing effectively. For the account names and types, default to using names that represent a global population of users, patients and clinicians. The hospital can be called "St. Luke's Hospital" (STLH). 
...
| Tier      | Shape                                                     | Price            |
| --------- | --------------------------------------------------------- | ---------------- |
| Practice  | up to ~5 clinicians, 1,000 patients                       | existing $399/mo |
| Mid-sized | up to 7 departments, up to ~12 clinicians, 5,000 patients | from $1,500/mo   |
| High      | 7-10 departments, ~30 clinicians, 15,000 patients         | from $3,000/mo   |



|             |                                                     |     |                |
| ----------- | --------------------------------------------------- | --- | -------------- |
| Enterprise+ | 10+ departments, 31+ clinicians or 15,000+ patients | —   | From $4,000/mo |
